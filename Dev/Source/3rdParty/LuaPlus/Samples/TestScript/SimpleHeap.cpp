...
/**
	Constructor.
**/
SimpleHeap::SimpleHeap(const char* name) :
	m_buffer(NULL),
	m_bufferSize(0),
	m_bottomBlock(NULL),
	m_topBlock(NULL),
		...
		const char* allocName, const char* fileName, int lineNumber)
{
	...
	long totalSize = size + OVERRUN_BYTES;
...
//	memset(newBlock->GetData(0), 0xfd, size);
...
void SimpleHeap::Free(void* ptr)
{
	...
	unsigned long originalTotalSize = toFreeBlock->GetSize();
#endif // SIMPLEHEAP_STATS
...
{
#ifdef SIMPLEHEAP_ALLOC_STATS
	static bool s_debugIntegrityCheckOn = true;
