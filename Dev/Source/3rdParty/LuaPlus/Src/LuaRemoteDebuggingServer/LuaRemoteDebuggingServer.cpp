...
static LuaObject GetValueObjectFromIdent( LuaState* luaStateToDebug, SimpleString ident )
{
	...
	while ( true )
	{
		...
		curTable = curTable[ findIdent ];
		if ( curTable.IsNil() )
			break;
	}

	// Retrieve the final object.
	LuaObject obj = curTable.IsNil() ? curTable : ( findIdent.IsEmpty() ? curTable : curTable[ findIdent ] );
							...
							LuaObject& curObj, SimpleString& watchStr, int indentLevel )
{
	...
	for ( LuaTableIterator it( curObj ); it; ++it )
	{
		...
		if ( keyObj.IsString() )
			keyValueInfo.m_origKeyIdent = keyObj.GetString();
...
static int LS_DebugAddLocalExpandIdent( LuaState* state )
{
	...
	LuaStack args(state);
	SimpleString expandIdent = args[2].GetString();
...
static int LS_DebugDoBuffer( LuaState* state )
{
	...
	lua_dobuffer( *luaStateToDebug, bufferToSend.GetString(), bufferToSend.StrLen(), LUA_DEBUGGER_SOCKET_NAME );
	return 0;
}
...
void LuaRemoteDebuggingServer::RegisterState(const char* stateName, LuaPlus::LuaState* state)
{
	LuaStateInfo* info = FindLuaStateInfo(stateName);
	if (info)
	{
...
/**
	ReadScriptFile() inserts a file into an internal buffer so the
