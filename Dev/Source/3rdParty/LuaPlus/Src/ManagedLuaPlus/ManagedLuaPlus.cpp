...
//	int DoWBuffer(const lua_WChar* buff, size_t size, const char *name);
	...
	LuaStackObject* NewUserDataBox_Stack(void* u){  return new LuaStackObject(this, m_state->NewUserDataBox_Stack(u));  }
	...
	LuaStackObject* GetDefaultMetaTable(int type){  return new LuaStackObject(this, m_state->GetDefaultMetaTable(type));  }
	void SetDefaultMetaTable(int type)									{  m_state->SetDefaultMetaTable(type);  }
