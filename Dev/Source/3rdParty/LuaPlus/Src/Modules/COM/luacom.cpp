...
// RCS Info
static char *rcsid = "$Id: luacom.cpp,v 1.49 2004/02/19 16:36:20 almendra Exp $";
...
{
  return LuaBeans::getBeans(L, MODULENAME, LCOM_LUACOM_TYPENAME);
}
                              ...
                              LuaBeans* lbeans)
{
  ...
  {
    lcom =
      tLuaCOM::CreateLuaCOM(iluacom, lbeans, clsid, interface_typeinfo);
  }
...
static int luacom_Connect(lua_State *L)
{
  ...
  try
  {
    ...
    server_disp = 
      tLuaDispatch::CreateLuaDispatch(pTypeinfo, ref, lbeans);

    lua_unref(L, ref);

    CHECKPOSCOND(server_disp);

    server = tLuaCOM::CreateLuaCOM(server_disp, lbeans);
...
static int luacom_ImplInterfaceFromTypelib(lua_State *L)
{
  ...
  try
  {
    ...
    lcom = 
      ImplInterface(typelib, clsid, pcInterface, ref, lbeans);
...
static int luacom_CreateObject(lua_State *L)
{
  ...
  try
  {
    ...
    lcom = tLuaCOM::CreateLuaCOM(pdisp, lbeans, clsid, NULL, untyped);
  }
  catch(class tLuaCOMException& e)
  {
    COM_RELEASE(psi);
...
static int luacom_GetObject(lua_State *L)
{
  ...
  try
  {
    ...
    else  // tests whether the user passed in a DisplayName of a moniker
    {
      ...
      hr = MkParseDisplayName(pbc, tUtil::string2bstr(progId), &chEaten, &pmk);
      CHK_COM_CODE(hr);
    ...
    lcom = tLuaCOM::CreateLuaCOM(pdisp, lbeans, clsid);
    CHECKPOSCOND(lcom);
  }
  catch(class tLuaCOMException& e)
  {
...
static int luacom_NewObject(lua_State *L)
{
  ...
  try
  {
    ...
    iluacom = 
      tLuaDispatch::CreateLuaDispatch(interface_typeinfo, ref, lbeans);

    lua_unref(L, ref);


    // Creates associated luacom object
    lcom = tLuaCOM::CreateLuaCOM(iluacom, lbeans, clsid);
...
static int luacom_GetEnumerator(lua_State *L)
{
  ...
  {
    retvals = 
      luacom->call(DISPID_NEWENUM, INVOKE_PROPERTYGET, NULL, tLuaObjList());
  ...
  try
  {
    ...
    if(lcom->hasTypeInfo())
    {
      ...
      else if(found)
      {
        ...
        lcom->call(funcinfo.propput->memid,
                   INVOKE_PROPERTYPUT,
                   funcinfo.propput,
                               ...
                               const char *field_name)
{
  ...
  // Now tries to get the FUNCDESC
  {
    ...
    {
      if(lcom->getConstant(field_name))
      {
...
static int call_event(lua_State *L)
{
  ...
  try
  {
    ...
    // retrieves LuaCOM object
    lcom = (tLuaCOM*) lbeans->from_lua(luacom_object_param);
    CHECK(lcom, INTERNAL_ERROR);

    retval = lcom->call(DISPID_VALUE,
...
LUACOM_API int luacom_IDispatch2LuaCOM(lua_State *L, void *pdisp_arg)
{
  ...
  try
  {
    lcom = tLuaCOM::CreateLuaCOM((IDispatch*)pdisp_arg, lbeans);
  }
  catch(class tLuaCOMException& e)
  {
    UNUSED(e);
...
LUACOM_API void luacom_open(lua_State *L)
{
  ...
  luaCompat_openlib(L, "com", functions_tb);  
...

  {
    ...
    lbeans->registerObjectEvents(obj_events);
    lbeans->registerPointerEvents(pointer_events);
  }
  ...
  // features of LuaCOM
#ifdef LUA5
#define LUA_DEBUGGING
#ifdef LUA_DEBUGGING
  lua_dostring(L, "require(\"luacom5\")");
#else
#include "luacom5.loh"
#endif
#endif
...
extern "C" LUAMODULE_API int luaopen_com(lua_State* L)
{
	OleInitialize(NULL);
	luacom_open(L);
	return 0;
