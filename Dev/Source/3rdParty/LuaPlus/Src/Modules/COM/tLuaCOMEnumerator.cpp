...
tLuaCOMEnumerator::tLuaCOMEnumerator(lua_State* L_param,
                                     LuaBeans* lbeans_param,
                                     IEnumVARIANT* pEV_param)
{
  CHECKPRECOND(L_param && lbeans_param && pEV_param);
  ...
  typehandler = new tLuaCOMTypeHandler(NULL, lbeans);
}
...
int tLuaCOMEnumerator::call_method(lua_State *L)
{
  ...
  try
  {
    retval = enumerator->callCOMmethod(method_name, user_first_param, num_params);
  }
...
int tLuaCOMEnumerator::callCOMmethod(const char *name, int first_param, int num_params)
{
  HRESULT hr = S_OK;
  ...
  if(strcmp(name, "Next") == 0)
  {
    ...
    for(counter = 0; counter < fetched; counter++)
      typehandler->com2lua(pVar[counter]);
  ...
  if(strcmp(name, "Clone") == 0)
  {
    ...
    tLuaCOMEnumerator* enumerator = new tLuaCOMEnumerator(L, lbeans, p_newEV);
