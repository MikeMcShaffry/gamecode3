...
tLuaVector::tLuaVector(LuaBeans *p_lbeans)
{
  initialized = false;
  lbeans = p_lbeans;
...
stkIndex tLuaVector::getindex(long * indices, unsigned long size)
{
...
tLuaVector::Type tLuaVector::lua2Type(stkIndex luaval)
{
...
//
bool tLuaVector::CreateTable(void)
{
  lua_newtable(L);
  ...
  case VECTOR:
    {
      ...
      {
        bool succeeded = vectors[index]->CreateTable();
...
void tLuaVector::setindex(stkIndex luaval,
                          long * indices,
                          unsigned long size,
                          const SAFEARRAYBOUND* bounds)
{
  ...
  if(size > 1)
  {
    ...
    vectors[index]->setindex(luaval, indices, size - 1, bounds);
                                          ...
                                          unsigned long num_dimensions)
{
  ...
  else
  {
...

    {
      ...
      for(i = 0; i < length; i++)
      {
        vectors[i] = new tLuaVector(lbeans);
...
void tLuaVector::InitVectorFromTable(stkIndex table)
{
  ...
  try
  {
    ...
    while(1)
    {
      ...
      if(lua_istable(L, luaval) && lbeans->from_lua(luaval) == NULL)
      {
        ...
        // obtem novo vetor
        vectors[length] = new tLuaVector(lbeans);

        vectors[length]->InitVectorFromTable(luaval);
...
void tLuaVector::InitVectorFromStack(stkIndex first_elem)
{
  CHECKPRECOND(!initialized);
