...
public:
    ...
    size_t Len() const								{  return GetStringData()->stringLen;  }
    ...
    char GetAt(size_t n) const						{  assert(n < GetStringData()->stringLen);  return m_string[n];  }
    char operator[](int n) const					{  assert(n < (int)GetStringData()->stringLen);  return m_string[n];  }
    char operator[](size_t n) const					{  assert(n < (int)GetStringData()->stringLen);  return m_string[n];  }
    void SetAt(size_t n, char ch)					{  assert(n < GetStringData()->stringLen);  CopyBeforeWrite();  m_string[n] = ch;  }

    operator const char*() const					{  return m_string;  }
#ifdef __WXWINDOWS__
//	operator wxString() const						{  return wxString(m_string, GetStringData()->stringLen);  }
#endif
	...
	AnsiString TrimLeft(char chTarget = 0) const;
	AnsiString TrimRight(char chTarget = 0) const;
