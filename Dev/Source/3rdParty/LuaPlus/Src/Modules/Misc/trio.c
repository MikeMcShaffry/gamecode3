...
#if defined(_MSC_VER)
#pragma warning(disable: 4100 4244 4127 4706 4701)
#endif // _MSC_VER
	   ...
	   int base)
{
  ...
  if (flags & FLAGS_FLOAT_G)
	...
	precision = 1;
      ...
      if ((number < 1.0E-4) || (number > trio_powl(base,
      ...
      else if (number < 1.0)
	{
	  ...
	  if (workNumber - trio_floorl(workNumber) < 0.001)
	    workNumber--;
	  zeroes = (int)trio_floorl(workNumber);
	}
    }
  ...
  for (i = 0; i < integerDigits; i++)
    {
      workNumber = trio_floorl(((integerNumber + integerAdjust) * dblIntegerBase));
