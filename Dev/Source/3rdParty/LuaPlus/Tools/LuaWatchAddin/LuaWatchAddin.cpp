...
void ReadMem( DEBUGHELPER *pHelper, const void* fromPtr, void* toPtr,
			 size_t size )
{
	SIZE_T numRead;
...
*/
const TObject *luaH_getnum (DEBUGHELPER* pHelper, Table *t, int key)
{
	Table tBuf;
	ReadMem(pHelper, t, &tBuf, sizeof(Table));
...
//           (node(t, lmod(cast(lu_hash, cast(ls_hash, n)), sizenode(t))))
		...
		Node nBuf;
		ReadMem(pHelper, n, &nBuf, sizeof(Node));
...
// lapi.c
//////////////////////////////////////////////////////////////////////////
TObject *negindex (DEBUGHELPER* pHelper, lua_State *L, int index)
{
	...
	{
		return (TObject*)luaH_getnum(pHelper, hvalue(registry(L)), -index - 40000);
	}
...
TObject *luaA_indexAcceptable (DEBUGHELPER* pHelper, lua_State *L, int index)
{
	if (index > 0)
	{
...
void ReadTObject( DEBUGHELPER *pHelper, lua_State* state, TObject& obj, char* pResult,
				  size_t max )
{
	char outBuf[1024];
	size_t maxOutBuf;
				  ...
				  size_t max )
{
	...
	srcO = luaA_indexAcceptable(pHelper, &luaState, stackIndex);
	if (!srcO)
	{
	...
	ReadMem( pHelper, srcO, &obj, sizeof(TObject) );

	ReadTObject(pHelper, state, obj, pResult, max);
}
...
void ReadLuaObject( DEBUGHELPER* pHelper, LuaObject& luaObject, char *pResult, size_t max )
{
	...
	ReadTObject( pHelper, luaState.m_state, luaObject.m_object, pResult, max );
}
...
void ReadLuaObject( DEBUGHELPER* pHelper, DWORD dwAddress, char *pResult, size_t max )
{
	...
	if (luaObject.m_object.tt < LUA_TNONE  ||  luaObject.m_object.tt >= LUA_NTYPES - 1)
	{
		g_swapEndian = true;
	}
...
extern "C" HRESULT WINAPI AddIn_LuaObject( DWORD dwAddress, DEBUGHELPER *pHelper, int nBase, BOOL bUniStrings, char *pResult, size_t max, DWORD reserved )
{
