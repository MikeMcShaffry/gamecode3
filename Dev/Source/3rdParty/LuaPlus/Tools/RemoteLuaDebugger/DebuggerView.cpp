...
void CDebuggerView::DebugSetLocation(const CStringA& stateToDebug, const CString& fileName, int lineNumber)
{
...
void CDebuggerView::DebugUpdateCallStack(const CStringA& stateNameToDebug)
{
	BYTE* buffer;
	int size;
...
void CDebuggerView::DebugAddAllWatches(const CStringA& stateNameToDebug)
{
	m_watchGrid.DebugAddAllWatches();
}
...
void CDebuggerView::DebugSendAllBreakpoints(const CStringA& stateNameToDebug)
{
	int count = m_breakpointInfo.GetSize();
	for (int i = 0; i < count; ++i)
	{
		BreakpointInfo& info = m_breakpointInfo[i];
		CString command;
		command.Format(_T("DebugSetBreakpoint('%s','%s',%d,true)"), info.m_fileName, info.m_lineNumber);
