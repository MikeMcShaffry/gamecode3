...
public:
	...
	void DebugSetLocation(const CStringA& stateToDebug, const CString& fileName, int lineNumber);
	int LS_DebugUpdateLocals( LuaState* state );
	int LS_DebugUpdateWatch( LuaState* state );
	void DebugUpdateCallStack(const CStringA& stateNameToDebug);
	void DebugAddAllWatches(const CStringA& stateNameToDebug);
	void DebugSendAllBreakpoints(const CStringA& stateNameToDebug);
	...
	const CStringA& GetStateToDebug() const			{  return m_stateToDebug;  }
