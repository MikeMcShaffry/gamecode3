...
//	SaveExpandInfo(true);
	...
	for ( int i = 0; i < tableCount; ++i )
	{
		...
		CString ident = entryObj[ 1 ].GetString();
		int indentLevel = indentLevelObj.GetInteger();
		CStringW value = DebugBasicSerialize( valueObj );
...
void CVariablesGrid::DebugAddAllWatches()
{
	...
	for (int i = 0; i < watchCount; ++i)
	{
		...
		CString command;
		command.Format(_T("DebugAddWatch('%s',%d,'%s',false)"), theApp.GetDebuggerView()->GetStateToDebug(), i, name);
...
int CVariablesGrid::AddEditProperty( int treeDepth, const CString& title, bool expanded )
{
	int curRow = GetRowCount();
	SetRowCount(curRow + 1);
	SetFont(&m_dataFont);
