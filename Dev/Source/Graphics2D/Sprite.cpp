...
HRESULT Sprite::VOnRestore()
{
	...
	return (D3DXCreateSprite(DXUTGetD3DDevice(), &m_d3dSprite)==S_OK);
}
...
HRESULT Sprite::VOnRender(double fTime, float fElapsedTime)
{
	...
	Vec3 translation(actualPos.x, actualPos.y, 0.0f);
...
HRESULT BitmapSprite::VOnRestore()
{
	...
	if (m_bInCache)
	{
		...
		if ( FAILED ( D3DXCreateTextureFromFileInMemoryEx( 
			DXUTGetD3DDevice(), // device
			textureBuffer,		// source data
			resource.m_size,	// source data size
	...
	assert(m_Height * m_NumFrames == imageInfo.Height && _T("Sprite height error"));
...
HRESULT TextSprite::VOnRestore()
{
	...
	assert(sizeTotal.cx > 0 && sizeTotal.cy > 0 && "About to create sprite with no width or height, is this what you want?");
	...
	// Create the texture
	if ( FAILED ( DXUTGetD3DDevice()->CreateTexture(
		texW,		// width 
		texH,	// height 
